...
function renderFilePreview(fileData) { // Function for File module preview
...
function renderCustomData(fileData) { // Function for custom data rendering
    ...
    if ('file_info' in fileData && fileData.file_info) {
        // User Rating
...
$(document).ready(function() {
    ...
    // --- Folder View ---
    socket.emit('emit_files_page_get_folders', {
            path: path, 
        }, (response) => { // event handler for folders
            console.log('emit_files_page_get_folders', response);

            const folderView = new FolderViewComponent(response, response.folder_path, true); // Enable context menu
    ...
    socket.emit('emit_files_page_get_files', {
      path: path, 
      ...
      temperature: search_state.temperature,
        ...
        console.log('emit_files_page_show_files', response);

        // Update or Initialize FileGridComponent
        const fileGridComponent = new FileGridComponent({
            containerId: '#files_grid_container', // Use the new container ID
            filesData: response.files_data,
            renderPreviewContent: renderFilePreview, // Pass the text preview function
    ...
    //     socket.emit('emit_files_page_save_file_content', { file_path: currentFilePath, text_content: textContent }); // Use currentFilePath
    //     $('#text_file_modal').removeClass('is-active'); // Hide modal after save
    // });


    // // Show file content in the modal
    // socket.on('emit_files_page_show_file_content', (data) => {
