...
(chunk_embeddings, user_rating) pairs from all user-rated video files.
"""
...
def get_training_pairs(cfg, text_embedder, status_callback=None):
    ...
    Uses the "metadata" strategy: generates a text description for each video
    ...
    if not media_dir or not os.path.isdir(media_dir):
        print("[videos/train] media_directory not configured or missing, skipping.")
        return

    try:
        entries = db_models.VideosLibrary.query.filter(
